...
const ScheduleGeneration: React.FC<ScheduleGenerationProps> = ({
  ...
  const handleReset = () => {
    if (confirm('Are you sure you want to reset all data? This will clear all faculty, subjects, classrooms, and the current schedule.')) {
      onResetData();
      toast.success('All data has been reset. You can now create a new timetable.');
    }
  };
    ...
    <div className={`${className} p-6 glass rounded-lg`}>
      ...
      <div className="space-y-3">
        ...
        <Button
          onClick={handleReset}
          variant="outline"
          className="w-full flex items-center gap-2 border-destructive/30 text-destructive hover:bg-destructive/10"
        >
          <Trash2 size={16} />
          Reset All Data
        </Button>
